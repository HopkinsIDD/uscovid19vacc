...
# Run functions to generat data

```{r}

# Get clean, but not age group standardized data
vacc_data <- get_clean_us_agevacc(git_token = "AB63TGCIYANL5EQ647HRXH3AYAWZ6") %>% suppressMessages()

# Make it standardized to either 5yr or 10 year age groups
vacc_data_10yr <- get_standardized_us_agevacc(vacc_data = vacc_data,
                                        age_groups = "10yr", 
                                        git_token = "AB63TGCIYANL5EQ647HRXH3AYAWZ6") %>% suppressMessages()
...
```{r}
usps_samp <- c("AR","DE","DC","IA","NY","LA","MT","NH","OK","PR","WY","WV")
...
```{r}
usps_samp <- c("FL","MN","MA","MD","HI","IL","MT","NJ","NE","NC","NM","NV","ND")
vacc_data_10yr %>% filter(USPS %in% usps_samp) %>%
              arrange(prop_vacc_age) %>%
              mutate(USPS = fct_reorder(USPS, (prop_vacc_age))) %>%
              ggplot(aes(x=date, y=prop_vacc_age, color=age_group, group=age_group)) +
...
7. ND - Drop "2021-03-25" and oldest group in "2021-04-02"
